...
| **Graph store** | **NetworkX** `MultiDiGraph` for the mutable ingest layer, persisted to SQLite; retrieval traverses an immutable **CSR** projection (kg/csr.py, NumPy/SciPy) rebuilt on store change | PageRank/BFS run on the CSR arrays (~16 B/edge vs a dict/edge); Leiden still materializes networkx on the cold communities path. (An earlier note named Kùzu as the outgrows-memory target — Kùzu was abandoned/archived Oct 2025; the scale path is now lazy CSR + SQLite, no new dependency.) |
